...
use iroh_blobs::{
    api::{
        ...
        proto::{
            ...
            ExportBaoRequest, ExportRangesItem, ExportRangesMsg, ExportRangesRequest,
            ImportBaoMsg, ImportBaoRequest, ImportByteStreamMsg, ImportByteStreamUpdate,
            ImportBytesMsg, ImportBytesRequest, ListBlobsMsg, ListTagsMsg, ListTempTagsMsg,
            ObserveMsg, ObserveRequest, RenameTagMsg, Scope, SetTagMsg, SetTagRequest,
...
impl<D: BlobDir> Ctx<D> {
    ...
    async fn register_complete(&self, hash: Hash) -> io::Result<()> {
        let data = self.dir.open(&data_name(&hash)).await?;
        let size = data.len()?;
        let obao_present = self
            .dir
            .list()
            .await?
            .contains(&obao_name(&hash));
...
async fn run_gc_cycle<D: BlobDir + 'static>(ctx: &Rc<Ctx<D>>, gc: &GcOptions) {
    ...
    if deleted > 0 {
        debug!("opfs-store gc: deleted {deleted} blobs ({} live)", live.len());
    }
}
...
async fn handle_command<D: BlobDir + 'static>(
    ...
    match cmd {
            Command::ImportBytes(ImportBytesMsg {
                inner: ImportBytesRequest { data, format, scope },
                tx,
                ..
            }) => {
                ...
                if let Err(e) = import_bytes(&ctx, data, scope, format, &tx).await {
                    ...
                    tx.send(AddProgressItem::Error(io::Error::other(e))).await.ok();
                }
            }
            Command::ImportByteStream(ImportByteStreamMsg { inner, tx, rx, .. }) => {
                let _guard = ctx.tasks.begin();
                if let Err(e) =
                    import_byte_stream(&ctx, inner.scope, inner.format, rx, &tx).await
                {
