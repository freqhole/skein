 ...
 *  blob shouldn't fail the whole mixdown. prefers an applied gain
 *  rendition over the original (see `effectiveAudioRef()`) — gain is
 *  already baked into its samples, so nothing further is needed here to
 *  reflect a clip's own volume adjustment in the mixdown. */
async function decodeClipAudio(
  ...
  if (!ref) return null; // e.g. a tts clip that hasn't been generated yet
  const cached = cache.get(ref.blobId);
  ...
  try {
    ...
    if (!url) {
      cache.set(ref.blobId, null);
      return null;
    }
    const res = await fetch(url);
    const bytes = await res.arrayBuffer();
    const decoded = await decodeCtx.decodeAudioData(bytes);
    cache.set(ref.blobId, decoded);
    return decoded;
  } catch (err) {
    console.warn(`[animaniac] audio-mixdown: failed to decode clip ${clip.id} (blob ${ref.blobId}):`, err);
    cache.set(ref.blobId, null);
    return null;
  }
}
